...
async function vizualizarInformacoesGlobais() {
    ...
    paragrafo.innerHTML = `Você sabia que o total de pessoas no mundo é de <span>${pessoasMundo}</span> de pessoas e que o  total de pessoas com acesso à educação é de <span>${pessoasComAcessoAEducacao}</span>? O tempo médio gasto por dia estudando é de <span> ${horas} horas</span> e <span>${minutos}</span> minutos e o total de pessoas com educação superior é de <span>${pessoasComEducacaoSuperior}</span>. Isso significa que aproximadamente <span>${percentual}%</span> de pessoas tem acesso à Educação. <br>Será que esses dados apontam para uma realidade de um mundo esclarecido e com facilidade de acesso à Educação?  
`
